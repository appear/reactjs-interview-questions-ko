...
2. ### What are the major features of ReactJS? (React의 특징은 무엇이 있을까요?)
...
4. ### What is the difference between Element and Component? (element와 component의 차이점은 무엇인가요?)
...
### 5. How to create components in ReactJS? (React에서 컴포넌트를 어떻게 생성하나요?)
...
### 6. When to use a Class Component over a Functional Component? (언제 클래스 컴포넌트를 사용하고 언제 Functional 컴포넌트를 사용할까요?)
...
### 7. What are Pure Components? (순수 컴포넌트란 무엇인가요?)
...
### 8. What is state in ReactJS? (state란 무엇인가요?)
...
### 9. What is props in ReactJS ? (props란 무엇인가요?)
...
### 10. What is the difference between state and props ? (state와 props의 차이점은 무엇인가요?)
...
### 11. Why should not we update the state directly? (state를 직접 업데이트 하면 안되는 이유는 무엇인가요?)
...
### 12. What is the purpose of callback function as an argument of setState? (setState에서 callback의 역할은 무엇인가요?)
...
### 13. What is the difference of event handling between HTML and React? (HTML과 React의 이벤트 처리 차이점은 무엇인가요?)
...
### 14. How to bind methods or event handlers in JSX callbacks? (Or) How to use this keyword in JSX callbacks? (This를 바인딩하는 방법)
3가지의 방법이 있습니다.
...
### 15. How to pass a parameter to an event handler or callback? (이벤트 핸들러 또는 콜백에 매개변수를 어떻게 전달하나요?)
...
### 16. What are synthetic events in ReactJS? (React에서의 합성 이벤트는 무엇인가요?)
...
### 17. What is inline conditional expressions? (인라인 조건식은 무엇인가요?)
...
### 18. What is Key and benefit of using it in lists? (list 에서 key를 사용했을 때의 이점은 무엇인가요?)
...
### 19. What is the use of create refs? (ref의 용도는 무엇인가요?)
두 가지의 접근법이 있습니다.
...
### 20. How to create refs? (create refs를 어떻게 만드나요?) 
